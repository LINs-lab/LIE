...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        # Don't add best_model path to previous_saved_paths to prevent it from being deleted
        # during regular checkpoint cleanup
        if "best_model" not in local_path:
            self.previous_saved_paths.append(local_path)
            print(f"length previous_saved_paths: {len(self.previous_saved_paths)}")
        else:
            log_with_rank(f"Skipping addition of best_model path to previous_saved_paths: {local_path}", rank=self.rank, logger=logger)
