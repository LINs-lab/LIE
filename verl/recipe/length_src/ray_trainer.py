...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, config=None):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        token_level_scores_repetition = data.batch['token_level_scores_repetition']
        ...
        ## handle repetition now
        repetition_normalized_score, _ = core_algos.compute_grpo_outcome_advantage(token_level_rewards=token_level_scores_repetition,
                                                                        response_mask=grpo_calculation_mask,
                                                                        index=data.non_tensor_batch["uid"],
                                                                        norm_adv_by_std_in_grpo=norm_adv_by_std_in_grpo,)
        
        new_advantage = correctness_normalized_score + length_normalized_score + repetition_normalized_score
...
class RayPPOTrainer:
    ...
    def _dump_generations(self, inputs, outputs, scores, reward_extra_infos_dict, dump_path, budget=None, sample_response_lengths=None):
        """Dump rollout/validation samples as JSONL."""
        os.makedirs(dump_path, exist_ok=True)
        filename = os.path.join(dump_path, f"{self.global_steps}_{budget}.jsonl")
        ...
        if sample_response_lengths is not None:
            base_data["length"] = sample_response_lengths
    ...
    def _validate(self):
        ...
        for test_data in tqdm(self.val_dataloader):
            ...
            assert test_batch.batch["responses"].shape[1] == budgets[-1], f"response length: {test_batch.batch['responses'].shape[1]}, budget: {budgets[-1]}"
        ...
        for budget in budgets:
                ...
                if "reward_model" in test_batch.non_tensor_batch:
                    ...
                    if isinstance(reward_model, dict):
                        ...
                        if hasattr(reward_model, 'shape') and isinstance(reward_model[0], dict):
                            ...
                            test_batch.non_tensor_batch["reward_model"] = [{'num_tokens': budget}]
            ...
            print("responses shape",test_output_gen_batch.batch["responses"].shape, "\nbudget:", budget)
            ...
            budget_metric_dict[f'avg_score/{budget}'] = np.mean([budget_metric_dict[key] for key in budget_metric_dict if 'mean@' in key and str(budget) in key and 'polaris' not in key])
            budget_metric_dict[f'avg_score/{budget}/four_sets'] = np.mean([budget_metric_dict[key] for key in budget_metric_dict if 'mean@' in key and str(budget) in key and ('aime' in key or 'olympiad_bench' in key or 'math' in key or 'amc' in key)])
