...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        kwargs = {
            "max_tokens": prompts.meta_info.get("max_tokens", self.config.response_length)
        }
        ...
        with self.update_sampling_params(**kwargs):
            ...
            max_tokens = kwargs.get("max_tokens", self.config.response_length)
            response = pad_2d_list_to_length(response, self.pad_token_id, max_length=max_tokens).to(idx.device)
            if self.config.calculate_log_probs:
                rollout_log_probs = pad_2d_list_to_length(rollout_log_probs, -1, max_length=max_tokens).to(idx.device)
