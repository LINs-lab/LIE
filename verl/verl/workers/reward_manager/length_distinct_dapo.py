...
from recipe.reward_ours.distinct_ngram import calculate_distinct_ngram_ratio
...
@register("length_distinct_dapo")
class LengthDistinctDAPORewardManager:
    """The reward manager."""
    ...
    def __call__(self, data: DataProto, return_dict: bool = False, reward_config: dict = {"length_penalty_type": None,
        ...
        "extra_tokens": 0,
        "distinct_ratio_penalty": True,
        "distinct_ratio_penalty_type": "hard", # "soft" or "hard"
        "distinct_ratio_threshold": 0.05,
        "distinct_ratio_score_penalty": 0.6,
        }
    ):
        ...
        distinct_ratio_tensor = torch.zeros_like(data.batch["responses"], dtype=torch.float32)
        ...
        for i in range(len(data)):
            ...
            if data.meta_info.get("validate", False) == True:
                ...
                if reward_config['length_penalty_type'] == 'remove_upper_refined':
                    ...
                    if reward_config["target_length_type"] == 'offline':
                        num_tokens = abs(data.non_tensor_batch["reward_model"][i]['num_tokens']) + reward_config["extra_tokens"]
                ...
                distinct_ratio = calculate_distinct_ngram_ratio(response_str, n=10)
                distinct_ratio_list.append(distinct_ratio)
                
                
                if reward_config.get("distinct_ratio_penalty", False):
                    target_value = 0.95
                    
                    threshold = reward_config.get("distinct_ratio_threshold", 0.05)
                    penalty_base = reward_config.get("distinct_ratio_score_penalty", 0.6)
                    # v1: 0.92 +- 0.05 之外给penalty
                    diff = abs(distinct_ratio - target_value)
                    # v2: <= 0.95 给penalty 
                    # diff = 0.95 - distinct_ratio
                    # threshold = 0

                    
                    if diff > threshold:
                        if reward_config.get("distinct_ratio_penalty_type", "soft") == "soft":
                            # 超过threshold的部分按比例扣分
                            distinct_ratio_penalty = -penalty_base * (diff - threshold) / threshold
                            distinct_ratio_penalty = max(-penalty_base, distinct_ratio_penalty)

                        else:
                            # Hard penalty
                            distinct_ratio_penalty = -penalty_base
                
                score += distinct_ratio_penalty
                distinct_ratio_penalty_list.append(distinct_ratio_penalty)
                
                      
                print(f"[INFO] distinct_ratio_penalty: {distinct_ratio_penalty}, distinct_ratio: {distinct_ratio}, length_penalty: {delta_score}, original_score: {original_score_list[i]}, final_score: {score}")
            ...
            distinct_ratio_tensor[i, valid_response_length - 1] = distinct_ratio_penalty
        ...
        reward_extra_info.update({"response_lengths": torch.tensor(response_lengths, dtype=torch.float32)})
        
        
        if len(distinct_ratio_list) > 0:
            reward_extra_info.update({"distinct_ratio_list": torch.tensor(distinct_ratio_list, dtype=torch.float32)})
            reward_extra_info.update({"distinct_ratio_penalty_list": torch.tensor(distinct_ratio_penalty_list, dtype=torch.float32)})
        ...
        if return_dict:
            return {
                ...
                "distinct_ratio_tensor": distinct_ratio_tensor,
