        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    loss_agg_mode_for_update = self.config.get("loss_agg_mode_for_update", loss_agg_mode)
                    ...
                    if self.config.policy_loss.loss_mode == "vanilla":
                        pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower = compute_policy_loss(
                            ...
                            loss_agg_mode=loss_agg_mode_for_update,
                        )
                    else:
                        policy_loss_fn = get_policy_loss_fn(loss_mode)
                        pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower = policy_loss_fn(old_log_prob, log_prob, advantages, response_mask, loss_agg_mode_for_update, self.config)

                    entropy_loss = agg_loss(loss_mat=entropy, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)

                    # # Log per-chunk entropy aggregated by the same `loss_agg_mode` used in training.
                    # # Baseline only: DO NOT apply any entropy bonus schedule here; we only observe entropy by chunk.
                    # max_chunks_to_log = int(self.config.get("entropy_chunk_metrics_max_chunks", 16))
                    # max_chunks_to_log = max(max_chunks_to_log, 0)
                    # if max_chunks_to_log > 0:
                    #     with torch.no_grad():
                    #         chunk_size = int(self.config.get("entropy_chunk_size", 512))
                    #         chunk_size = max(chunk_size, 1)

                    #         valid_mask = response_mask.to(torch.bool)
                    #         if valid_mask.any():
                    #             token_index = torch.cumsum(valid_mask.to(torch.long), dim=1) - 1
                    #             token_index = torch.clamp(token_index, min=0)
                    #             chunk_id = token_index // chunk_size  # (bsz, response_length)

                    #             observed_max_chunk = int(chunk_id[valid_mask].max().item())
                    #             chunks_to_log = min(max_chunks_to_log, observed_max_chunk + 1)
                    #             for c in range(chunks_to_log):
                    #                 chunk_mask = valid_mask & (chunk_id == c)
                    #                 if not chunk_mask.any():
                    #                     continue
                    #                 chunk_mask_f = chunk_mask.to(response_mask.dtype)
                    #                 ent_chunk = agg_loss(
                    #                     loss_mat=entropy,
                    #                     loss_mask=chunk_mask_f,
                    #                     loss_agg_mode=loss_agg_mode,
                    #                 )
                    #                 append_to_dict(
                    #                     metrics,
                    #                     {f"actor/entropy_loss_chunk/{c}": ent_chunk.detach().item()},
                    #                 )
                    if entropy_coeff != 0:
                        # compute policy loss
